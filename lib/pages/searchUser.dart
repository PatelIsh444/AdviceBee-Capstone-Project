...
class searchUser extends SearchDelegate<String>{
  ...
  getUserData(s)async{
    await Firestore.instance.collection("users").where("displayId", whereIn: s).getDocuments()
    .then((docs){
      print(docs);
    });
  ...
  Widget buildResults(BuildContext context) {
    Stream<QuerySnapshot> stream = Firestore.instance.collection('users').snapshots();
    return StreamBuilder(
      ...
      builder: (BuildContext context,AsyncSnapshot<QuerySnapshot> snapshot) {
        if (!snapshot.hasData) {
          ...
          suggestions=snapshot.data.documents.where((d) => d["displayName"].toString().toLowerCase().contains(query)).toList();
