...
//          print(dateJoined);
     ...
     return AlertDialog(
         ...
         content: SingleChildScrollView(
             child: ListBody(
                 children: <Widget>[
                   Text(
                       "You have $scores points, get to $pointAmount and you will"
   ...
   AlertDialog promotedMessage(BuildContext context, String pointAmount) {
     ...
     if (pointAmount == "500") {
       ...
       rankName = "Queen Bee";
       nextRankName =
       "You have proven yourself as the biggest bee in the hive! "
    ...
    visited*/
     if (croppedImage != null) {
       ...
       FirebaseStorage().ref().child(
           "profilePictures/" + CurrentUser.userID + "profilePicture");
       final StorageUploadTask uploadTask = pictureNameInStorage.putFile(
           _image);
       ...
       File(_image.path)
         ..writeAsBytesSync(im.encodePng(thumbnailImage));

       //Upload thumbnail
       final StorageReference thumbnailPictureNameInStorage =
       FirebaseStorage().ref().child(
           "profilePictures/" + CurrentUser.userID + "profileThumbnailPicture");
       final StorageUploadTask uploadTask2 = thumbnailPictureNameInStorage
           .putFile(_image);
       await uploadTask2.onComplete;

       var imageURL = await pictureNameInStorage.getDownloadURL() as String;
       var thumbnailImageURL = await thumbnailPictureNameInStorage
